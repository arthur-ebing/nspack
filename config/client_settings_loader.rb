class AppClientSettingsLoader # rubocop:disable Metrics/ClassLength
  REPLACE_RULES = { # These should become client rules!
    ...
    DELIVERY_DEFAULT_RMT_CONTAINER_TYPE: { env_key: 'DEFAULT_RMT_CONTAINER_TYPE',
                                           required: true,
                                           desc: 'This should be set behind the scenes, since it will almost always be the same thing, e.g. ‘bins’, for a given packhouse.' },
    ...
    DELIVERY_CAPTURE_TRUCK_AT_FRUIT_RECEPTION: { env_key: 'CAPTURE_TRUCK_AT_FRUIT_RECEPTION',
                                                 boolean: true,
                                                 desc: 'Optional capture at delivery' },
    USE_PERMANENT_RMT_BIN_BARCODES: { env_key: 'USE_PERMANENT_RMT_BIN_BARCODES',
                                      boolean: true,
                                      desc: 'Capture deliveries with permanently barcoded bins.' },
  ...
  RULES = {
    ...
    PRESORT1_MSSQL_SERVER_INTERFACE: { env_key: 'PRESORT1_MSSQL_SERVER_INTERFACE',
                                       desc: 'HTTP address of the MsSql Interface Server for presort plant 1 in the format http://IP:8080. It is used in the bin_staging service.' },
    PRESORT2_MSSQL_SERVER_INTERFACE: { env_key: 'PRESORT2_MSSQL_SERVER_INTERFACE',
                                       desc: 'HTTP address of the MsSql Interface Server for presort plant 2 in the format http://IP:8086. It is used in the bin_staging service.' },
    BIN_TIPPED_MSSQL_SERVER_INTERFACE: { env_key: 'BIN_TIPPED_MSSQL_SERVER_INTERFACE',
                                         desc: 'HTTP address of the MsSql Interface Server in the format http://IP:8080. It is used in the bin_tipped service.' },
                                       desc: 'HTTP address of the MsSql Interface Server for presort plant 2 in the format http://IP:8086.' },
    BIN_CREATED_MSSQL_SERVER_INTERFACE: { env_key: 'BIN_CREATED_MSSQL_SERVER_INTERFACE',
                                          desc: 'HTTP address of the MsSql Interface Server in the format http://IP:8080. This is used in the bin_created service' },
