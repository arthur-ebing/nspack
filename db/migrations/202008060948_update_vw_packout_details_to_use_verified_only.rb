Sequel.migration do
  up do
    run <<~SQL
      create or replace view vw_packout_details
            (farm_code, orchards, pack_date, pack_week, deliveries, no_of_bins, total_bin_weight, total_packed_weight,
             total_pallet_weight, run_carton_percentage, run_pallet_percentage, cultivar, marketing_variety_code,
             production_run_id, production_run_code, grade_code, packhouse, line, basic_pack, std_size, size_ref,
             actual_count, inventory_code, nett_weight, carton_quantity, percentage, derived_nett_weight)
      as
      SELECT farms.farm_code,
             ...
             CASE production_run_stats.bins_tipped_weight
                 ...
                 ELSE (production_run_stats.cartons_verified_weight / production_run_stats.bins_tipped_weight *
                       100::numeric)::numeric(7, 2)
                 END                                                                          AS run_carton_percentage,
             CASE production_run_stats.bins_tipped_weight
                 WHEN 0 THEN 0.0::numeric(7, 2)
                 ELSE (production_run_stats.pallet_weight / production_run_stats.bins_tipped_weight *
                       100::numeric)::numeric(7, 2)
             ...
             CASE production_run_stats.cartons_verified_weight
                 WHEN 0 THEN 0.0::numeric(7, 2)
                 ELSE (pallet_sequences.nett_weight / production_run_stats.cartons_verified_weight *
                       100::numeric)::numeric(7, 2)
                 END                                                                          AS percentage,
      ...
      FROM pallet_sequences
               ...
               LEFT JOIN standard_product_weights ON commodities.id = standard_product_weights.commodity_id AND
                                                     standard_product_weights.standard_pack_id = standard_pack_codes.id
               ...
               LEFT JOIN fruit_actual_counts_for_packs
                         ON fruit_actual_counts_for_packs.id = pallet_sequences.fruit_actual_counts_for_pack_id
      ...
      WHERE pallet_sequences.pallet_id IS NOT NULL AND pallet_sequences.verified IS TRUE;
      
      alter table vw_packout_details
          owner to postgres;
    SQL
  end
