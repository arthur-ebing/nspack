Sequel.migration do
  up do
    ...
    alter_table(:colour_percentages) do
      set_column_type :colour_percentage, String
    end

    alter_table(:rmt_bins) do
      add_foreign_key :colour_percentage_id, :colour_percentages
    end

    alter_table(:rmt_sizes) do
      add_foreign_key :std_fruit_size_count_from_id, :std_fruit_size_counts
      add_foreign_key :std_fruit_size_count_to_id, :std_fruit_size_counts
    end
  ...
  down do
    ...
    alter_table(:rmt_sizes) do
      drop_foreign_key :std_fruit_size_count_from_id
      drop_foreign_key :std_fruit_size_count_to_id
    end

    alter_table(:rmt_bins) do
      drop_foreign_key :colour_percentage_id
    end

    # NB: the conversion back from string to int needs to be done in SQL with the USING clause:
    run <<~SQL
      ALTER TABLE colour_percentages ALTER COLUMN colour_percentage TYPE integer USING (trim(regexp_replace(colour_percentage, '[^0-9]+', '', 'g'))::integer);
    SQL
