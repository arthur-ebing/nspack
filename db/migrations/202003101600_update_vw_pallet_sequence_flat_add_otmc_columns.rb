    ...
    run <<~SQL
      ...
      CREATE OR REPLACE VIEW public.vw_pallet_sequence_flat AS 
       SELECT ps.id,
          ...
          p.edi_in_consignment_note_number,
          COALESCE(p.govt_reinspection_at, p.govt_first_inspection_at) AS inspection_date,
          COALESCE(p.edi_in_consignment_note_number, CASE WHEN NOT (p.govt_inspection_passed) THEN
            LPAD(govt_inspection_pallets.govt_inspection_sheet_id::text,10, '0')||'F'
            WHEN (p.govt_inspection_passed) THEN LPAD(govt_inspection_pallets.govt_inspection_sheet_id::text,10, '0')
            ELSE
             ''
            END) AS addendum_manifest
         FROM pallets p
