Sequel.migration do
  up do
    alter_table(:pallets) do
      add_column :last_govt_inspection_pallet_id, Integer
    end
    run <<~SQL
      ...
      CREATE OR REPLACE VIEW public.vw_pallet_sequence_flat AS
        SELECT ps.id,
          ...
          (ps.carton_quantity::NUMERIC / p.carton_quantity) AS pallet_size,
          p.build_status,
        ...
        FROM pallets p
        LEFT JOIN govt_inspection_pallets on govt_inspection_pallets.pallet_id=p.last_govt_inspection_pallet_id
