  ...
  up do
    run <<~SQL
      CREATE OR REPLACE VIEW public.vw_rmt_bins_flat AS
        SELECT
            rmt_bins.id,
            COALESCE(bin_asset_number, tipped_asset_number, shipped_asset_number, scrapped_bin_asset_number) AS asset_number,
            fn_current_status ('rmt_bins', rmt_bins.id) AS status,
            rmt_bins.qty_bins,
            ...
            fn_party_role_name (rmt_bins.rmt_material_owner_party_role_id::int) AS container_material_owner,
            --
            ...
            EXTRACT(WEEK FROM rmt_bins.bin_tipped_date_time) as bin_tipped_week,
            --
