    ...
    run <<~SQL
      ...
      CREATE OR REPLACE VIEW public.vw_cartons AS
        SELECT
            ...
            carton_labels.created_at AS carton_label_created_at,
            abs(date_part('epoch'::text, CURRENT_TIMESTAMP - carton_labels.created_at) /
               3600::double precision)::integer AS label_age_hrs,
            abs(date_part('epoch'::text, CURRENT_TIMESTAMP - cartons.created_at) /
               3600::double precision)::integer AS carton_age_hrs,
            concat(contract_workers.first_name, '_', contract_workers.surname) AS contract_worker,
            cartons.created_at AS carton_verified_at,
            packhouses.plant_resource_code AS packhouse,
            lines.plant_resource_code AS line,
            ...
            basic_pack_codes.basic_pack_code,
            standard_pack_codes.standard_pack_code,
            fn_party_role_name(cartons.marketing_org_party_role_id) AS marketer,
            marks.mark_code,
            inventory_codes.inventory_code,
            cartons.product_resource_allocation_id AS resource_allocation_id,
            product_setup_templates.template_name AS product_setup_template,
            pm_boms.bom_code AS pm_bom,
            (SELECT array_agg(t.treatment_code ORDER BY t.treatment_code) AS array_agg
            FROM treatments t
            JOIN cartons cl ON t.id = ANY (cl.treatment_ids)
            WHERE cl.id = cartons.id
            GROUP BY cl.id) AS treatment_codes,
            cartons.client_size_reference AS client_size_ref,
            cartons.client_product_code,
            cartons.marketing_order_number,
            target_market_groups.target_market_group_name AS packed_tm_group,
            seasons.season_code,
            pm_subtypes.subtype_code,
            pm_types.pm_type_code,
            ...
            cartons.pick_ref,
            cartons.pallet_sequence_id,
            COALESCE(cartons.pallet_number, (SELECT pallet_sequences.pallet_number
        ...
        LEFT JOIN system_resources ON packpoints.system_resource_id = system_resources.id
        JOIN farms ON farms.id = cartons.farm_id
        JOIN pucs ON pucs.id = cartons.puc_id
        JOIN orchards ON orchards.id = cartons.orchard_id
        ...
        LEFT JOIN customer_varieties ON customer_varieties.id = cartons.customer_variety_id
        LEFT JOIN marketing_varieties cvv ON cvv.id = customer_varieties.variety_as_customer_variety_id
        ...
        JOIN standard_pack_codes ON standard_pack_codes.id = cartons.standard_pack_code_id
        JOIN marks ON marks.id = cartons.mark_id
        JOIN inventory_codes ON inventory_codes.id = cartons.inventory_code_id
        LEFT JOIN pm_boms ON pm_boms.id = cartons.pm_bom_id
        LEFT JOIN pm_subtypes ON pm_subtypes.id = cartons.pm_subtype_id
        LEFT JOIN pm_types ON pm_types.id = cartons.pm_type_id
        JOIN target_market_groups ON target_market_groups.id = cartons.packed_tm_group_id
        JOIN seasons ON seasons.id = cartons.season_id
        JOIN cartons_per_pallet ON cartons_per_pallet.id = cartons.cartons_per_pallet_id
        LEFT JOIN pm_products ON pm_products.id = cartons.fruit_sticker_pm_product_id
        JOIN pallet_formats ON pallet_formats.id = cartons.pallet_format_id
        JOIN contract_workers ON contract_workers.id = cartons.contract_worker_id
        LEFT JOIN personnel_identifiers ON personnel_identifiers.id = cartons.personnel_identifier_id
        JOIN packing_methods ON packing_methods.id = cartons.packing_method_id
        LEFT JOIN personnel_identifiers palletizers ON palletizers.id = cartons.palletizer_identifier_id
        LEFT JOIN contract_workers palletizer_contract_workers ON palletizer_contract_workers.id = cartons.palletizer_contract_worker_id;      
        ALTER TABLE public.vw_cartons OWNER TO postgres;
    SQL
