...
Sequel.migration do
  up do
    run "UPDATE product_setups SET grade_id = 1 WHERE grade_id IS NULL;
         UPDATE carton_labels SET grade_id = 1 WHERE grade_id IS NULL;
         UPDATE cartons SET grade_id = 1 WHERE grade_id IS NULL;"
    alter_table(:product_setups) do
      set_column_not_null :grade_id
    end
    ...
    run "UPDATE pallet_sequences SET grade_id = 1 WHERE grade_id IS NULL;"
    alter_table(:pallet_sequences) do
      set_column_not_null :grade_id
    end
    run <<~SQL
  ...
  down do
    ...
    SQL
    alter_table(:product_setups) do
      set_column_allow_null :grade_id
    end
    alter_table(:carton_labels) do
      set_column_allow_null :grade_id
    end
