Sequel.migration do
  up do
    run <<~SQL
      ...
      CREATE VIEW public.vw_pallet_sequence_flat AS 
        SELECT
          ps.id,
          ps.pallet_id,
          ps.pallet_number,
          ...
          to_char(ps.created_at, 'IYYY--IW') AS packed_week,
          ps.created_at,
          ps.updated_at,
          p.scrapped,
          p.scrapped_at,
          ...
          (SELECT array_agg(t.treatment_code)
          ...
          COALESCE(pod_voyage_ports.ata, pod_voyage_ports.eta)::date AS arrival_date,
          COALESCE(pod_voyage_ports.atd, pod_voyage_ports.etd)::date AS departure_date,
          ...
          CASE
            WHEN NOT (p.govt_inspection_passed)
              THEN fn_consignment_note_number(govt_inspection_sheets.id) || 'F'
            WHEN (p.govt_inspection_passed)
              THEN fn_consignment_note_number(govt_inspection_sheets.id)
            ELSE
              ''
        ...
        LEFT JOIN (SELECT sq.id,
                     coalesce( trim( array_agg(orchard_test_types.test_type_code)::TEXT),'')<>'' AS failed_otmc,
                   ...
                   GROUP BY sq.id) AS otmc ON otmc.id = ps.id
        ORDER BY
            ps.pallet_number,
            ps.pallet_sequence_number;

      ALTER TABLE public.vw_pallet_sequence_flat OWNER TO postgres;
    SQL
  end
    ...
    run <<~SQL
      ...
      CREATE VIEW public.vw_pallet_sequence_flat AS 
        SELECT
          ...
          CASE
            WHEN NOT (p.govt_inspection_passed)
              THEN LPAD(govt_inspection_pallets.govt_inspection_sheet_id::text, 10, '0') || 'F'
            WHEN (p.govt_inspection_passed)
              THEN LPAD(govt_inspection_pallets.govt_inspection_sheet_id::text, 10, '0')
            ELSE
              ''
          END) AS addendum_manifest,
          p.repacked,
