Sequel.migration do
  up do
    if ENV['CLIENT_CODE'] != 'kr'
      ...
      # Sync Group login to MesScada
      # ----------------------------
      run <<~SQL
        CREATE OR REPLACE FUNCTION kromco_legacy.sync_group_login_to_messcada()
            RETURNS trigger
            LANGUAGE 'plpgsql'
            COST 100
        ...
        BEGIN
          IF (NEW.from_external_system <> true AND NEW.active) THEN -- ONLY handle group login, logout ignored - handled by script
            ...
            INSERT INTO kromco_legacy.messcada_people_group_members(
              reader_id, rfid, industry_number, group_id, group_date, module_name, module_name_alias, last_name, first_name, title, person_role, from_external_system)
              SELECT '1', p.identifier, c.personnel_number, p_group_code, p_date, p_module, p_module, c.surname, c.first_name, c.title, r.packer_role, true
            ...
            LEFT OUTER JOIN public.contract_worker_packer_roles r ON r.id = c.packer_role_id;
