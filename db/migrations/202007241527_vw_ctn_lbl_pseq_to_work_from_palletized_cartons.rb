      ...
      CREATE OR REPLACE VIEW public.vw_carton_label_pseq
       ...
       SELECT pallet_sequences.id,
         ...
         FROM pallet_sequences
           ...
           -- CASE stmt to work with carton-palletized cartons as well as non.
           -- The view can return multiple records for palletized ctns, but only the 1st record is used.
           JOIN cartons ON CASE WHEN pallet_sequences.scanned_from_carton_id IS NULL THEN cartons.pallet_sequence_id = pallet_sequences.id ELSE cartons.id = pallet_sequences.scanned_from_carton_id END
