    ...
    run <<~SQL
      ...
      CREATE OR REPLACE VIEW public.vw_pallet_sequence_flat AS
        SELECT ps.id,
          ...
          COALESCE(p.gross_weight, load_containers.actual_payload / (SELECT COUNT(*) FROM pallets WHERE load_id = loads.id)) AS gross_weight,
          p.gross_weight_measured_at,
          p.nett_weight,
          ps.nett_weight AS sequence_nett_weight,
          coalesce (p.gross_weight / p.carton_quantity, load_containers.actual_payload / (SELECT SUM(carton_quantity) FROM pallets WHERE load_id = loads.id)) AS carton_gross_weight,
          ps.nett_weight / ps.carton_quantity AS carton_nett_weight,
          p.exit_ref,
          p.phc,
          p.stock_created_at,
          p.intake_created_at,
          ...
          CASE
              WHEN p.gross_weight IS NULL THEN 
                (ps.nett_weight / (SELECT SUM(nett_weight) FROM pallet_sequences WHERE pallet_id IN (SELECT id FROM pallets WHERE load_id = loads.id))) * load_containers.actual_payload 
              ELSE 
                (ps.carton_quantity::numeric / p.carton_quantity::numeric) * p.gross_weight
