Sequel.migration do
    ...
    run <<~SQL
      CREATE OR REPLACE VIEW public.vw_pallet_sequence_flat AS
        SELECT
          ...
          (SELECT array_agg(clt.treatment_code) FROM
          (SELECT t.treatment_code FROM treatments t JOIN pallet_sequences cl ON t.id = ANY (cl.treatment_ids) WHERE cl.id = ps.id ORDER BY t.treatment_code DESC) AS clt) AS treatments,
          ...
          p.build_status,
          fn_current_status('pallets', p.id) AS status,
          ...
          CASE WHEN NOT p.in_stock THEN NULL 
            ...
            WHEN p.shipped THEN 'inactive'
            WHEN p.scrapped THEN 'warning'
            WHEN p.inspected AND NOT p.govt_inspection_passed  THEN 'error'
            WHEN fn_pallet_verification_failed(p.id) THEN 'error'
            WHEN ps.verified AND NOT ps.verification_passed THEN 'error'
            WHEN p.palletized OR p.partially_palletized THEN 'inprogress'
            ELSE NULL 
          END AS colour_rule
        FROM pallets p
        ...
        JOIN plant_resources lines ON lines.id = ps.production_line_resource_id
  ...
  down do
    run 'DROP VIEW public.vw_pallet_sequence_flat;'
  end
end
