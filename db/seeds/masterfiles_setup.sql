...
-- PROGRAM: Quality
INSERT INTO programs (program_name, program_sequence, functional_area_id)
VALUES ('Quality', 1, (SELECT id FROM functional_areas WHERE functional_area_name = 'Masterfiles'));

-- LINK program to webapp
INSERT INTO programs_webapps(program_id, webapp) VALUES (
      (SELECT id FROM programs
       WHERE program_name = 'Quality'
         AND functional_area_id = (SELECT id FROM functional_areas
                                   WHERE functional_area_name = 'Masterfiles')),
       'Nspack');

-- LIST menu item
-- PROGRAM FUNCTION Pallet_verification_failure_reasons
INSERT INTO program_functions (program_id, program_function_name, url, program_function_sequence)
VALUES ((SELECT id FROM programs WHERE program_name = 'Quality'
         AND functional_area_id = (SELECT id FROM functional_areas
                                   WHERE functional_area_name = 'Masterfiles')),
         'Pallet Verification Failure Reasons', '/list/pallet_verification_failure_reasons', 2);
