...
INSERT INTO reworks_run_types (run_type, description) VALUES('BULK REBIN RUN UPDATE', 'Bulk Rebin Run Update') ON CONFLICT DO NOTHING;
...
-- move node permission
INSERT INTO security_permissions (security_permission) VALUES('move_node') ON CONFLICT DO NOTHING;
INSERT INTO security_groups (security_group_name) VALUES('move_node') ON CONFLICT DO NOTHING;

INSERT INTO security_groups_security_permissions (security_group_id, security_permission_id)
SELECT (SELECT id FROM security_groups WHERE security_group_name = 'move_node'), p.security_permission_id
FROM security_groups_security_permissions p
WHERE p.security_group_id = (SELECT id FROM security_groups WHERE security_group_name = 'basic') ON CONFLICT DO NOTHING;

INSERT INTO security_groups_security_permissions (security_group_id, security_permission_id)
SELECT g.id,
(SELECT id FROM security_permissions WHERE security_permission = 'move_node')
FROM security_groups g WHERE g.security_group_name = 'move_node' ON CONFLICT DO NOTHING;
