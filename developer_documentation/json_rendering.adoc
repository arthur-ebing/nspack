...
Like with updating a grid row, this will close the dialog and append a row to the underlying grid.
    ...
    add_grid_row(attrs:, notice: nil, at_start: false)
...
If `at_start` is true, the new row will be inserted at the top of the grid. Useful when a grid is sorted in decending order.
