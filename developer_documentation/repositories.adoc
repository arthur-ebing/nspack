...
[source,ruby]
----
repo.for_select_commodities
----
Resulting in this SQL:
[source,sql]
----
SELECT "code", "id" FROM "commodities" WHERE "active" ORDER BY "code"
----

The results can be restricted by providing a `:where` and/or an `:exclude` clause:

```ruby
# WHERE
repo.for_select_commodities where: { commodity_group_id: 123 }
# Resulting in this SQL:
# SELECT "code", "id" FROM "commodities"
#  WHERE ("active" AND ("commodity_group_id" = 123))
# ORDER BY "code"

# WHERE NOT
repo.for_select_commodities exclude: { commodity_group_id: [5, 6, 7] }
# Resulting in this SQL:
# SELECT "code", "id" FROM "commodities"
#  WHERE ("active" AND ("commodity_group_id" NOT IN (5, 6, 7)))
# ORDER BY "code"
```
