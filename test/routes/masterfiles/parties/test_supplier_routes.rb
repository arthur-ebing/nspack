...
class TestSupplierRoutes < RouteTester
  ...
  def test_edit
    authorise_pass! permission_check: MasterfilesApp::TaskPermissionCheck::Supplier
  ...
  def test_delete_fail
    authorise_pass! permission_check: MasterfilesApp::TaskPermissionCheck::Supplier
    ensure_exists!(INTERACTOR)
    INTERACTOR.any_instance.stubs(:delete_supplier).returns(bad_response)
    delete_as_fetch 'masterfiles/parties/suppliers/1', {}, 'rack.session' => { user_id: 1, last_grid_url: DEFAULT_LAST_GRID_URL }
    expect_json_error
  end
  ...
  def test_new
    ...
    Masterfiles::Parties::Supplier::New.stub(:call, bland_page) do
      get  'masterfiles/parties/suppliers/new', {}, 'rack.session' => { user_id: 1 }
    end
    expect_bland_page
  end
  ...
  def test_create_remotely
    ...
    ensure_exists!(INTERACTOR)
    row_vals = Hash.new(1)
    INTERACTOR.any_instance.stubs(:create_supplier).returns(ok_response(instance: row_vals))
    post_as_fetch 'masterfiles/parties/suppliers', {}, 'rack.session' => { user_id: 1, last_grid_url: DEFAULT_LAST_GRID_URL }
