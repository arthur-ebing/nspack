...
module Production
  module Dashboards
    module Dashboard
      class CartonPalletSummaryWeeks
        def self.call
          layout = Crossbeams::Layout::Page.build({}) do |page|
            page.add_text 'Carton/Pallet Summary per week', wrapper: :h2
        ...
        def self.draw_boxes # rubocop:disable Metrics/AbcSize
          ...
          recs = calculate_summary(week_no, ctn_recs, plt_recs)
          ...
          ar << <<~HTML
            <div class="flex flex-column mv2 bg-mid-gray pt1 pl2 pb2 outline">
              <div class="flex flex-wrap near-white">
                <div class="pa2 mr2 mt2 fw8"> Week #{week_no}</div>
              </div>
          ...
          prev_week = week_no == 1 ? 52 : week_no - 1
          prev_recs = calculate_summary(prev_week, ctn_recs, plt_recs)
          unless prev_recs.empty?
            ar << <<~HTML
              <div class="flex flex-column mv3 bg-mid-gray pt1 pl2 pb2 outline">
                <div class="flex flex-wrap near-white">
                  <div class="pa2 mr2 mt2 fw8"> Week #{prev_week}</div>
                </div>
        ...
        def self.calculate_summary(week_no, ctn, plt) # rubocop:disable Metrics/AbcSize, Metrics/CyclomaticComplexity
          # Do for last 2 days
          ctn_recs = ctn.select { |r| r[:week] == week_no } # Date.today } # 2020-07-20
          plt_recs = plt.select { |r| r[:week] == week_no } # Date.today }
          key_set = Set.new
