      ...
      class DeliveryWeeks
        def self.call
          layout = Crossbeams::Layout::Page.build({}) do |page|
            page.add_text 'Deliveries per week', wrapper: :h2
        ...
        def self.draw_boxes
          recs = ProductionApp::DashboardRepo.new.deliveries_per_week
          tots = ProductionApp::DashboardRepo.new.delivery_cultivars_per_week
        ...
        def self.week_items(recs) # rubocop:disable Metrics/AbcSize
          recs.map do |rec|
            percentage = if rec[:qty_tipped].zero? || rec[:qty_bins].zero?
                           0
                         else
