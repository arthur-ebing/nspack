...
module Production
  module Dashboards
    module Dashboard
      class PalletsInStockSize
        def self.call
          layout = Crossbeams::Layout::Page.build({}) do |page|
            page.add_text 'Pallets in stock per size', wrapper: :h2
        ...
        def self.draw_boxes
          recs = ProductionApp::DashboardRepo.new.pallets_in_stock_per_size
        ...
        def self.cultivar_totals(recs)
          lkp = {}
          recs.each do |rec|
            lkp[[rec[:cultivar_name], rec[:count_swap_rule]]] ||= 0
            lkp[[rec[:cultivar_name], rec[:count_swap_rule]]] += rec[:pallet_count]
          end
          lkp.map do |grp, count|
            <<~HTML
              <div class="outline pa2 mr3 mt2 tc" style="min-width:230px;background:#e6f4f1;">
                <p class="fw6 f4 mt0 pb1 bb">#{grp.first} | #{grp.last}</p>
          ...
          recs.map do |rec|
            <<~HTML
              <div class="outline pa2 mr3 mt2 bg-white" style="min-width:230px">
                <p class="fw6 mt0 pb1 bb">TM <span class="f4">#{rec[:packed_tm_group]}</span> | Size <span class="f4">#{rec[:count_swap_rule]}</span></p>
