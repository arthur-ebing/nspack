...
module UiRules
  class FarmSectionRule < Base
    ...
    def set_show_fields
      ...
      fields[:farm_manager_party_role_id] = { renderer: :label,
                                              with_value: farm_manager_party_role_id_label,
                                              caption: 'Farm Manager' }
      fields[:farm_section_name] = { renderer: :label }
      fields[:description] = { renderer: :label }
      fields[:orchard_ids] = { renderer: :list,
                               caption: 'Orchards',
                               items: @repo.for_select_orchards(where: { id: @form_object.orchard_ids }) }
    end

    def common_fields
      {
        farm_section_name: { required: true },
        farm_manager_party_role_id: { renderer: :select,
                                      options: MasterfilesApp::PartyRepo.new.for_select_party_roles(AppConst::ROLE_FARM_MANAGER),
        ...
        orchard_ids: { renderer: :multi,
                       options: @repo.for_select_orchards(where: { farm_id: @options[:farm_id], farm_section_id: nil }) + @repo.for_select_orchards(where: { id: @form_object.orchard_ids }),
                       caption: 'Orchards',
                       selected: @form_object.orchard_ids,
                       required: true  }
      }
    end
