...
module MesscadaApp
  class NewPalletSequence < BaseService
    ...
    def validate_pallet_mix_rules(new_sequence) # rubocop:disable Metrics/AbcSize, Metrics/CyclomaticComplexity, Metrics/PerceivedComplexity
      ...
      return failed_response("Pallet cultivar_name:#{oldest_sequence[:cultivar_name]}. You are adding a sequence with cultivar_name: #{repo.get(:cultivars, new_sequence[:cultivar_id], :cultivar_name)} . Mixing is not allowed") if !rule[:allow_cultivar_mix] && (new_sequence[:cultivar_id] != oldest_sequence[:cultivar_id])
      return failed_response("Pallet cultivar_group_code:#{oldest_sequence[:cultivar_group_code]}. You are adding a sequence with cultivar_group_code: #{repo.get(:cultivar_groups, new_sequence[:cultivar_group_id], :cultivar_group_code)} . Mixing is not allowed") if !rule[:allow_cultivar_group_mix] && (new_sequence[:cultivar_group_id] != oldest_sequence[:cultivar_group_id])
